...
class ScaledParameterisedActionWrapper(gym.ActionWrapper):
    ...
    def __init__(self, env):
        ...
        self.num_conti_actions = self.old_as.spaces.__len__() - 1
        self.high = [self.old_as.spaces[i].high for i in range(1, self.num_conti_actions + 1)]
        self.low = [self.old_as.spaces[i].low for i in range(1, self.num_conti_actions + 1)]
        self.range = [self.old_as.spaces[i].high - self.old_as.spaces[i].low for i in range(1, self.num_conti_actions + 1)]
        new_params = [  # parameters
            Box(-np.ones(self.old_as.spaces[i].low.shape), np.ones(self.old_as.spaces[i].high.shape), dtype=np.float32)
            for i in range(1, self.num_conti_actions + 1)
        ]
    ...
    def action(self, action):
        ...
        for a in range(1, len(action)):
            scale = float(self.range[a - 1])
            action[a] = scale * (action[a] + 1) / 2. + float(self.low[a - 1])
        return action
